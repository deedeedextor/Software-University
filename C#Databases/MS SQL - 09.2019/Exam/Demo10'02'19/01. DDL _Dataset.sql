...
---Problem 10
SELECT TOP(1) sp.[Name] AS [Spaceshipname],
              spr.Name AS [SpaceportName]
FROM Spaceships AS sp
JOIN Journeys AS j ON j.SpaceshipId = sp.Id
JOIN Spaceports AS spr ON spr.Id = j.DestinationSpaceportId
...
FROM Planets AS p
JOIN Spaceports AS spp ON p.Id = spp.PlanetId
JOIN Journeys AS j ON spp.Id = j.DestinationSpaceportId
...
JOIN Journeys AS j ON spp.Id = j.DestinationSpaceportId
GROUP BY p.Name
ORDER BY COUNT(j.Id) DESC, [PlanetName]
...
WHERE JourneyId = (SELECT TOP(1) Id FROM Journeys AS j
...
FROM (SELECT tc.JobDuringJourney,
	  ...
	  GROUP BY tc.JobDuringJourney, c.BirthDate, tc.ColonistId 
     ) AS k
...
FROM Planets AS p
LEFT JOIN Spaceports AS spp ON p.Id = spp.PlanetId
GROUP BY p.Name
...
BEGIN 
      RETURN(SELECT COUNT(*) FROM Journeys AS j
	         JOIN Spaceports AS spp ON j.DestinationSpaceportId = spp.Id
			 JOIN Planets AS p ON spp.PlanetId = p.Id
...
BEGIN
	...
	DECLARE @targetPurpose VARCHAR(30) = (SELECT j.Purpose
	                                      FROM Journeys AS j
										  WHERE j.Id = @JourneyId)
...
BEGIN 
	 ...
	 SELECT Id,JourneyStart,JourneyEnd,Purpose,DestinationSpaceportId,
		SpaceshipId FROM deleted
END
GO
